...
export class FusionSwapContract {
    ...
    create(order, accounts) {
        ...
        return new TransactionInstruction(this.programId, [
            {
                // 0. system_program
            ...
            {
                // 4. escrow_src_ata
                pubkey: getAta(escrow, order.srcMint, accounts.srcTokenProgram),
                isWritable: true,
                isSigner: false
            },
            {
            ...
            order.srcAssetIsNative
                ? this.OPTIONAL_ACCOUNT_META
                : {
                    pubkey: getAta(accounts.maker, order.srcMint, accounts.srcTokenProgram),
                    isWritable: true,
                    isSigner: false
                },
            {
        ...
        return new TransactionInstruction(this.programId, [
            ...
            },
            {
                ...
                pubkey: accounts.takerSrcAccount ??
                    getAta(accounts.taker, order.srcMint, accounts.srcTokenProgram),
                isWritable: true,
                isSigner: false
            },
            {
            ...
            order.dstAssetIsNative
                ? this.OPTIONAL_ACCOUNT_META
                : {
                    pubkey: getAta(order.receiver, order.dstMint, accounts.dstTokenProgram),
                    isWritable: true,
                    isSigner: false
                },
            // 14. taker_dst_ata
            order.dstAssetIsNative
                ? this.OPTIONAL_ACCOUNT_META
                : {
                    pubkey: getAta(accounts.taker, order.dstMint, accounts.dstTokenProgram),
                    isWritable: true,
                    isSigner: false
                },
            // 15. protocol_dst_ata
    ...
    rewardLimit = order.resolverCancellationConfig?.maxCancellationPremium) {
        assert(order.resolverCancellationConfig, 'order can not be cancelled by resolver');
